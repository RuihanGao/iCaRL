...
# Load the dataset
print("Loading data...")
val_ratio = 0.2 # val/(train+val)
data = utils_slide610.load_slide_data(val_ratio)
...
for iteration_total in range(nb_runs):
    ...
    [network,intermed] = utils_slide610.build_cnn(input_var, n)
    ...
    Y_protoset_cumuls = []
    Y_train_cumuls    = []
    alpha_dr_herding  = np.zeros((int(nb_tot_cl/nb_cl),dictionary_size,nb_cl),np.float32)
    ...
    prototypes = np.zeros((nb_tot_cl,dictionary_size,X_train_total.shape[1],X_train_total.shape[2],X_train_total.shape[3]))
    for orde in range(nb_tot_cl):
        prototypes[orde,:,:,:,:] = X_train_total[np.where(Y_train_total==order[orde])]
    
    for iteration in range(int(nb_tot_cl/nb_cl)):
        ...
        for epoch in range(epochs):
            ...
            for batch in utils_slide610.iterate_minibatches(X_train, Y_train, batch_size, shuffle=True, augment=True):
                ...
                if (train_batches%10) == 1:
                    print("trianing batch {}: error {}".format(train_batches, train_err-old_train))
            ...
            for batch in utils_slide610.iterate_minibatches(X_valid, Y_valid, min(max_train_sample,len(X_valid)), shuffle=False):
                ...
                for i in range(inputs.shape[0]):
                    top5_acc = top5_acc+np.float((pred_ranked[i,targets_prep[i]]>=(nb_tot_cl-5)))/inputs.shape[0]
                    top1_acc = top1_acc+np.float((pred_ranked[i,targets_prep[i]]>=(nb_tot_cl-1)))/inputs.shape[0]
                
                val_err     += err
                val_batches += 1
            # Then we print the results for this epoch:
            print("Batch of classes {} out of {} batches".format(
                iteration + 1, int(nb_tot_cl/nb_cl)))
            print("Epoch {} of {} took {:.3f}s".format(
                epoch + 1, epochs, time.time() - start_time))
            print("  training loss:\t\t{:.6f}".format(train_err / train_batches))
            print("  validation loss:\t\t{:.6f}".format(val_err / val_batches))
            print("  top 1 accuracy:\t\t{:.2f} %".format(
                top1_acc / val_batches * nb_tot_cl))
            print("  top 5 accuracy:\t\t{:.2f} %".format(
                top5_acc / val_batches * nb_tot_cl))
        ...
        if iteration==0:
            [network2,intermed2] = utils_slide610.build_cnn(input_var, n)
        ...
        # Save the network
        np.savez('net_incr'+str(iteration+1)+'_of_'+str(int(nb_tot_cl/nb_cl))+'.npz', *lasagne.layers.get_all_param_values(network))
        ...
        class_means = np.zeros((64,nb_tot_cl,2))
        ...
        top1_acc_list_ori   = utils_slide610.accuracy_measure(X_valid_ori, Y_valid_ori, class_means, val_fn, top1_acc_list_ori, iteration, iteration_total, 'original')
        
        # Calculate validation error of model on the cumul of classes:
        print('Computing cumulative accuracy...')
        top1_acc_list_cumul = utils_slide610.accuracy_measure(X_valid_cumul, Y_valid_cumul, class_means, val_fn, top1_acc_list_cumul, iteration, iteration_total, 'cumul of')
